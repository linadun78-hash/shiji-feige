...
本仓库的自有源代码以“源码公开，保留权利”为发布方式。除非仓库后续明确添加书面许可证，否则不授予复制、修改、再发布、分发或商业使用许可；查看和克隆源码不等于取得上述授权。请通过 GitHub Issues 申请特定使用许可，项目方会单独确认。
...
后续顺序：完善本地连接与首次接入、提供经过验证的 Windows 整合包，再上架 Edge 和 Chrome。当前仓库自有代码尚未指定开源许可证；第三方组件保留各自许可证。本次发布定位为源码预览，不附带未经项目所有者确认的开放授权承诺。
